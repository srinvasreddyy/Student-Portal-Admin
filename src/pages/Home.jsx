...
import {
    ...
    Building, MailCheck, ClipboardList, TrendingUp,
    Globe, Network, PieChart, ChevronRight, Server,
    Lock, Fingerprint, Activity
} from 'lucide-react';

// ==========================================
// 0. CUSTOM RESPONSIVE HOOK
// ==========================================
const useIsMobile = () => {
...
const FEATURES_DATA = [
    { title: "Military-Grade Security", desc: "JWT-based authentication, AES-256 encryption, and strict role-based access controls ensure your corporate data remains impenetrable.", icon: <Lock size={32} strokeWidth={1.5} />, color: "#0ea5e9" },
    { title: "Complete Project Lifecycle", desc: "Seamlessly track project performance, manage applicant funnels, and monitor deliverables from proposal to final execution.", icon: <ClipboardList size={32} strokeWidth={1.5} />, color: "#2563eb" },
    { title: "Automated Trust Verification", desc: "Instant credibility established through automated cross-referencing of official institutional and company registry data.", icon: <CheckCircle2 size={32} strokeWidth={1.5} />, color: "#0284c7" },
    { title: "Global Talent Network", desc: "Gain unrestricted access to a curated, pre-verified pool of the brightest minds from top universities worldwide.", icon: <Globe size={32} strokeWidth={1.5} />, color: "#3b82f6" },
    { title: "Advanced Analytics", desc: "Generate comprehensive reports on talent acquisition ROI, project engagement rates, and university partnership metrics.", icon: <PieChart size={32} strokeWidth={1.5} />, color: "#0369a1" },
    { title: "High-Availability Infrastructure", desc: "Built on a scalable microservices architecture guaranteeing 99.99% uptime for your critical recruitment operations.", icon: <Server size={32} strokeWidth={1.5} />, color: "#0c4a6e" }
];
...
const globalStyles = `
    ...
    /* Added box-sizing to prevent padding from causing horizontal overflow */
    *, *::before, *::after {
    ...
    return (
        ...
        >
            <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', textDecoration: 'none' }}>
                ...
                <span className="outfit-font" style={{ fontWeight: 800, fontSize: isMobile ? '1.2rem' : '1.5rem', color: '#0f172a', letterSpacing: '-0.5px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '1rem' : '2rem' }}>
                ...
                <div style={{ display: 'flex', gap: isMobile ? '0.75rem' : '1rem', alignItems: 'center' }}>
                    <Link to="/login" style={{ color: '#0f172a', textDecoration: 'none', fontWeight: 700, fontSize: isMobile ? '0.85rem' : '0.95rem' }}>
                        Login
                    </Link>
                    <Link to="/company" style={{ padding: isMobile ? '0.5rem 1rem' : '0.75rem 1.5rem', borderRadius: '12px', background: '#0f172a', color: '#ffffff', textDecoration: 'none', fontWeight: 600, fontSize: isMobile ? '0.85rem' : '0.95rem', transition: 'all 0.3s', border: '1px solid #1e293b' }} onMouseEnter={e => { e.currentTarget.style.background = '#0ea5e9'; e.currentTarget.style.borderColor = '#0ea5e9'; }} onMouseLeave={e => { e.currentTarget.style.background = '#0f172a'; e.currentTarget.style.borderColor = '#1e293b'; }}>
                        Partner
                    </Link>
                </div>
            </div>
...
const HeroSection = () => {
    const isMobile = useIsMobile();
    return (
        <div className="mesh-background" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', paddingTop: isMobile ? '100px' : '80px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 5%', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? '3rem' : '4rem', alignItems: 'center', width: '100%' }}>

                <motion.div variants={staggerContainer} initial="hidden" animate="show" style={{ zIndex: 10 }}>
                    <motion.div variants={fadeUpVariant} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1.2rem', background: '#ffffff', border: '1px solid #e0f2fe', borderRadius: '50px', marginBottom: '1.5rem', boxShadow: '0 4px 15px rgba(14,165,233,0.1)' }}>
                    ...
                    <motion.h1 variants={fadeUpVariant} className="outfit-font" style={{ fontSize: 'clamp(2.5rem, 6vw, 4.5rem)', fontWeight: 900, lineHeight: 1.1, color: '#0f172a', marginBottom: '1.5rem', letterSpacing: '-1px' }}>
                        Empower Your <br />
                        <span className="text-gradient">Organization's Future.</span>
                    </motion.h1>

                    <motion.p variants={fadeUpVariant} style={{ fontSize: 'clamp(1rem, 2vw, 1.15rem)', color: '#475569', lineHeight: 1.7, marginBottom: '2.5rem', maxWidth: isMobile ? '100%' : '90%' }}>
                    ...
                    <motion.div variants={fadeUpVariant} style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap', width: '100%' }}>
                        <Link to="/company" style={{ width: isMobile ? '100%' : 'auto', justifyContent: 'center', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '1.1rem 2.2rem', borderRadius: '14px', background: 'linear-gradient(135deg, #0ea5e9, #0284c7)', color: '#ffffff', textDecoration: 'none', fontWeight: 600, fontSize: '1.1rem', boxShadow: '0 15px 30px -10px rgba(14, 165, 233, 0.5)', transition: 'transform 0.2s' }}>
                            <Building size={20} /> Corporate Access
                        </Link>
                        <Link to="/university" style={{ width: isMobile ? '100%' : 'auto', justifyContent: 'center', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '1.1rem 2.2rem', borderRadius: '14px', background: '#ffffff', border: '1px solid #cbd5e1', color: '#0f172a', textDecoration: 'none', fontWeight: 600, fontSize: '1.1rem', transition: 'all 0.2s' }}>
                ...
                {!isMobile && (
                    ...
                    >
                        <motion.div
                            ...
                            style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%) rotateY(-15deg) rotateX(5deg)', width: '110%', maxWidth: '600px', padding: '1.5rem', zIndex: 2, background: 'rgba(255,255,255,0.85)' }}
                        >
...
const MockupRenderer = ({ type }) => {
    switch (type) {
            ...
            return (
                <div style={{ width: '100%', height: '100%', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div style={{ width: '100%', height: '48px', background: '#f1f5f9', borderRadius: '12px', border: '2px solid #0ea5e9', display: 'flex', alignItems: 'center', padding: '0 1rem', gap: '1rem' }}>
                        ...
                        <span style={{ color: '#0f172a', fontWeight: 500, fontSize: '0.9rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Acme Corp International...</span>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {[1, 2, 3].map(i => (
                            <div key={i} style={{ padding: '1rem', background: i === 1 ? '#e0f2fe' : '#ffffff', border: i === 1 ? '1px solid #bae6fd' : '1px solid #f1f5f9', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                <div style={{ width: '40px', height: '40px', background: i === 1 ? '#0ea5e9' : '#e2e8f0', borderRadius: '8px', flexShrink: 0 }} />
                                <div style={{ flex: 1 }}>
                                    <div style={{ width: '60%', height: '12px', background: i === 1 ? '#0f172a' : '#cbd5e1', borderRadius: '4px', marginBottom: '8px' }} />
                                    <div style={{ width: '40%', height: '8px', background: i === 1 ? '#64748b' : '#e2e8f0', borderRadius: '4px' }} />
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', zIndex: 2 }}>
                        ...
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#0f172a', marginBottom: '0.5rem' }}>Authenticating</div>
                        </div>
                    </div>
                </div>
            );
                ...
                <div style={{ width: '100%', height: '100%', padding: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ width: '100%', maxWidth: '300px', background: '#ffffff', border: '1px solid #e0f2fe', borderRadius: '16px', padding: '1.5rem', boxShadow: '0 20px 40px -10px rgba(2,132,199,0.15)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem', borderBottom: '1px solid #f1f5f9', paddingBottom: '1rem' }}>
                            <div style={{ width: '40px', height: '40px', background: '#0284c7', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><MailCheck size={20} color="#fff" /></div>
                            <div style={{ overflow: 'hidden' }}>
                                <div style={{ fontSize: '0.9rem', fontWeight: 700, color: '#0f172a' }}>Invitation</div>
                                <div style={{ fontSize: '0.75rem', color: '#64748b', textOverflow: 'ellipsis', overflow: 'hidden' }}>admin@acme.com</div>
                            </div>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                            ...
                            <div style={{ width: '100%', padding: '0.8rem', background: '#0284c7', color: '#fff', textAlign: 'center', borderRadius: '8px', fontSize: '0.85rem', fontWeight: 700 }}>Accept</div>
                        </div>
                    </div>
                </div>
            );
        case 'deploy':
            return (
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(circle, rgba(16,185,129,0.1) 0%, transparent 70%)', padding: '1rem' }}>
...
const WorkflowSection = () => {
    ...
    return (
        <div id="process" style={{ padding: isMobile ? '4rem 5%' : '8rem 5%', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
                <div style={{ textAlign: 'center', marginBottom: isMobile ? '2.5rem' : '5rem' }}>
                    ...
                    <h2 className="outfit-font" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, color: '#0f172a', margin: 0, lineHeight: 1.2 }}>
                        From Registration to <span style={{ color: '#0ea5e9' }}>Operation.</span>
                    </h2>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1.2fr', gap: isMobile ? '2rem' : '4rem', alignItems: 'center' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {DEMO_STEPS.map((step, idx) => (
                            <div
                                ...
                                style={{
                                    padding: '1.2rem', borderRadius: '20px', cursor: 'pointer', transition: 'all 0.3s ease',
                                    ...
                                    transform: activeStep === idx && !isMobile ? 'translateX(10px)' : 'none'
                                }}
                            >
                                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                                    <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: activeStep === idx ? step.color : '#e2e8f0', color: activeStep === idx ? '#fff' : '#94a3b8', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.3s' }}>
                                        {step.icon}
                                    </div>
                                    <div>
                                        <h3 className="outfit-font" style={{ fontSize: '1.1rem', fontWeight: 700, color: activeStep === idx ? '#0f172a' : '#64748b', margin: '0 0 0.5rem', transition: 'color 0.3s' }}>{step.title}</h3>
                                        <AnimatePresence initial={false}>
                                            {activeStep === idx && (
                                                <motion.p
                                                    ...
                                                    style={{ color: '#475569', fontSize: '0.9rem', lineHeight: 1.5, margin: 0, overflow: 'hidden' }}
                                                >
                                                    {step.desc}
                                                </motion.p>
                                            )}
...
const FeaturesSection = () => {
    const isMobile = useIsMobile();
    return (
        <div id="features" style={{ padding: isMobile ? '4rem 5%' : '8rem 5%', background: '#ffffff', position: 'relative' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
                <div style={{ textAlign: 'center', marginBottom: isMobile ? '3rem' : '5rem' }}>
                    ...
                    <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="outfit-font" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, color: '#0f172a', margin: 0, lineHeight: 1.2 }}>
                        Engineered for <span style={{ color: '#0ea5e9' }}>Enterprise.</span>
                    </motion.h2>
                    <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} style={{ color: '#475569', fontSize: '1.05rem', maxWidth: '750px', margin: '1.5rem auto 0', lineHeight: 1.6 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: '2rem' }}>
                    {FEATURES_DATA.map((feature, idx) => (
                        <motion.div
                            ...
                            whileHover={{ y: -5, boxShadow: `0 15px 30px -10px ${feature.color}20`, borderColor: `${feature.color}50` }}
                            style={{ background: '#ffffff', border: '1px solid #f1f5f9', borderRadius: '24px', padding: '2rem', transition: 'all 0.3s ease', display: 'flex', flexDirection: 'column', gap: '1.2rem', position: 'relative', overflow: 'hidden' }}
                        >
                            ...
                            <div style={{ position: 'relative', zIndex: 1, width: '56px', height: '56px', borderRadius: '16px', background: `${feature.color}10`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: feature.color }}>
                                {feature.icon}
                            </div>
                            <div style={{ position: 'relative', zIndex: 1 }}>
                                <h3 className="outfit-font" style={{ fontSize: '1.2rem', fontWeight: 700, color: '#0f172a', margin: '0 0 0.8rem' }}>{feature.title}</h3>
                                <p style={{ color: '#64748b', fontSize: '0.95rem', lineHeight: 1.6, margin: 0 }}>{feature.desc}</p>
                            </div>
                        </motion.div>
                    ))}
                </div>
...
const StatsSection = () => {
    return (
        <div style={{ padding: '4rem 5%', background: '#0f172a', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ maxWidth: '1400px', margin: '0 auto', position: 'relative', zIndex: 10, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 200px), 1fr))', gap: '2rem', textAlign: 'center' }}>
                {METRICS_DATA.map((stat, idx) => (
                    <motion.div
                        ...
                        style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', padding: '1.5rem', background: 'rgba(30, 41, 59, 0.5)', backdropFilter: 'blur(10px)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.05)' }}
                    >
                        <span className="outfit-font text-gradient" style={{ fontSize: '2.5rem', fontWeight: 900, lineHeight: 1 }}>{stat.value}</span>
                        <span style={{ color: '#cbd5e1', fontSize: '0.9rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px' }}>{stat.label}</span>
                    </motion.div>
                ))}
            </div>
        </div>
...
const CTASection = () => {
    const isMobile = useIsMobile();
    return (
        <div style={{ padding: isMobile ? '4rem 5%' : '8rem 5%', background: '#ffffff' }}>
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                <motion.div
                    ...
                    viewport={{ once: true }}
                    style={{
                        background: 'linear-gradient(135deg, #0ea5e9, #2563eb)', borderRadius: '32px', padding: isMobile ? '2.5rem 1.5rem' : '5rem 3rem',
                ...
                >
                    ...
                    <div style={{ position: 'relative', zIndex: 10 }}>
                        <h2 className="outfit-font" style={{ fontSize: 'clamp(2rem, 6vw, 4rem)', fontWeight: 800, color: '#ffffff', margin: '0 0 1rem', lineHeight: 1.2 }}>
                            Modernize Your Institutional <br /> Administration.
                        </h2>
                        <p style={{ color: '#e0f2fe', fontSize: isMobile ? '1rem' : '1.2rem', maxWidth: '650px', margin: '0 auto 2.5rem', lineHeight: 1.6 }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                            <Link to="/company" style={{ width: isMobile ? '100%' : 'auto', justifyContent: 'center', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '1.2rem 2rem', borderRadius: '16px', background: '#0f172a', color: '#ffffff', textDecoration: 'none', fontWeight: 700, fontSize: '1rem', transition: 'all 0.3s', boxShadow: '0 15px 30px rgba(15,23,42,0.3)' }}>
                                Register as Company <ChevronRight size={20} />
                            </Link>
                            <Link to="/university" style={{ width: isMobile ? '100%' : 'auto', justifyContent: 'center', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '1.2rem 2rem', borderRadius: '16px', background: '#ffffff', color: '#0ea5e9', textDecoration: 'none', fontWeight: 700, fontSize: '1rem', transition: 'all 0.3s' }}>
                                Register as University <ChevronRight size={20} />
                            </Link>
                        </div>
                    </div>
                </motion.div>
...
const Footer = () => {
    const isMobile = useIsMobile();
    return (
        <footer style={{ background: '#f8fafc', paddingTop: isMobile ? '3rem' : '4rem', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 5%' }}>
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(200px, 1fr))', gap: '2.5rem', marginBottom: '3rem' }}>

                    <div>
                        <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', textDecoration: 'none', marginBottom: '1.2rem' }}>
                    ...
                    <div>
                        <h4 className="outfit-font" style={{ color: '#0f172a', fontWeight: 700, fontSize: '1rem', marginBottom: '1.2rem' }}>Solutions</h4>
                        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                            {['University Portals', 'Corporate Dashboards', 'Security & Compliance', 'API Integrations', 'Case Studies'].map(item => (
                                <li key={item}><a href="#" style={{ color: '#64748b', textDecoration: 'none', fontSize: '0.9rem' }}>{item}</a></li>
                            ))}
                        </ul>
                    </div>

                    <div>
                        <h4 className="outfit-font" style={{ color: '#0f172a', fontWeight: 700, fontSize: '1rem', marginBottom: '1.2rem' }}>Company</h4>
                        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                    ...
                    <div>
                        <h4 className="outfit-font" style={{ color: '#0f172a', fontWeight: 700, fontSize: '1rem', marginBottom: '1.2rem' }}>Enterprise Support</h4>
                        <p style={{ color: '#64748b', fontSize: '0.9rem', lineHeight: 1.6, marginBottom: '1rem' }}>Dedicated account managers available 24/7 for premium tier partners.</p>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', color: '#0f172a', fontWeight: 600, fontSize: '0.9rem' }}>
                            <MailCheck size={18} color="#0ea5e9" /> support@adminconnect.io
                        </div>
                    </div>
                </div>

                <div style={{ padding: '1.5rem 0', borderTop: '1px solid #e2e8f0', display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'center' : 'center', textAlign: isMobile ? 'center' : 'left', gap: '1rem' }}>
                    <p style={{ color: '#94a3b8', fontSize: '0.85rem', margin: 0 }}>&copy; {new Date().getFullYear()} AdminConnect Enterprise. All rights reserved.</p>
                    <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', justifyContent: 'center' }}>
                        {['Privacy Policy', 'Terms of Service', 'Security Overview'].map(item => (
                            <a key={item} href="#" style={{ color: '#94a3b8', textDecoration: 'none', fontSize: '0.85rem' }}>{item}</a>
                        ))}
                    </div>
                </div>
            </div>
